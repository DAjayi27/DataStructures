...
public class DirectedGraph<T extends Comparable<T>> implements Graph<T> {
     ...
     * */
    public Vertex<T> getRandVertex(){
        Random random = new Random();
        Object[] values = vertexList.values().toArray();
        return (Vertex<T>) values[random.nextInt(vertexList.size()-1)];
    }
    ...
    /**
     *
     * Adds a one way connection between the 2 vertices . A connects to B, but B doesn't connect to A
    ...
    public void addEdge(Vertex<T> a , Vertex<T> b){
        if (!vertexList.containsValue(a)){
            throw new NoSuchElementException("vertex not in the graph");
        }

        if (!vertexList.containsValue(b)){
            throw new NoSuchElementException("vertex not in the graph");
        }
        ...
        boolean isConnected = a.getAdjacencies().contains(b);
        if (isConnected){
            return;
        }
    ...
    /**
     * Adds a one way connection between the 2 vertices . A connects to B, but B doesn't connect to A
     * I also adds a weight to the connection
     * If they are already connected, no action is preformed
     * If the two vertices are the same, no action is preformed.
     * @throws NoSuchElementException if one of the 2 vertices is not in the graph
     * **/
     ...
     * It also removes all its adjacency and connections to all adjacency
     *
    ...
    private void dijkstraHelper( Vertex<T> v , Set visited ){
       ...
       v.distance = 0;
       PriorityQueue<Integer,Vertex<T>> pq = new PriorityQueue<>();
